...
def sigmoid_coeffs(m,species):
    ...
    """
    Deaths simply cause the coefficients to be taken from the left of function
    """
    ...
    if n == 0: 
        ...
        x = np.arange(1,n+1)
        return 2* (-1/(1+a[species]**(-x+b[species])) + 1) + 0.1
...
def percent_IGT(IGT,species):
    ...
    if species == 'G':
        ...
        IGT[IGT > 100] = (np.log(IGT[IGT > 100])/np.log(1.5) - np.log(100)/np.log(1.5))*6.0 + 10
    elif species == 'R':
...
if __name__ == '__main__':
    ...
    coeffs = sigmoid_coeffs(int(16*m),species)
    IGT = np.sum(values**coeffs,axis = 1)
    IGT_ = percent_IGT(IGT, species)
