...
x_range = np.linspace(0,len(values[0])-1,len(values[0]))
plt.plot(x_range,mean_distribution)

#see how we can plot a fitted distribution
from scipy.optimize import curve_fit
params = curve_fit(lambda t,a,b: a*np.exp(b*t), x_range, mean_distribution, p0 = (4,0.5))

A = params[0][0]
b = params[0][1]

pred = A*np.exp(b*x_range)
...
coeffs = 2 / ((mean_distribution + 2)**2)
