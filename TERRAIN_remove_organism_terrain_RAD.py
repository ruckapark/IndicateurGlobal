...
def count_nonzero(df,xrange):
    ...
    """
    Return counts
    count(x) is the percentage of nonzero values > x
    """
    counts = []
...
def delete_counter(vals, window = 1):
    ...
    """
    Returns the highest counter with a window
    0001000 will return all zeros with window 1
    0001100 will return 4 with window 1 and stop at 11
    """
...
df_dist[df_dist > 100] = 0
df_train_vivant = sub_df(df_dist, 17, animals)[[1,3,5,6,7,10,11,12,13,15,16]]
df_train_mort = sub_df(df_dist, 17, animals)[[2,4,8,9,14]]
...
for i in df_train_vivant.columns:
    max_zero3.append(delete_counter(df_train_vivant[i].values))

for i in df_train_mort.columns:
    max_zero4.append(delete_counter(df_train_mort[i].values))
    

#check longest streak of non zero values:
df_rand_vi = pd.DataFrame(np.random.randn(df_train_vivant.shape[0],df_train_vivant.shape[1]),columns = df_train_vivant.columns, index = df_train_vivant.index)
df_rand_mo = pd.DataFrame(np.random.randn(df_train_mort.shape[0],df_train_mort.shape[1]),columns = df_train_mort.columns, index = df_train_mort.index)

# make zeros random numbers and non-zeros 1
df_counts_vi = (df_train_vivant > 0)
df_counts_mo = (df_train_mort > 0)

df_counts_vi[~df_counts_vi] = df_rand_vi[~df_counts_vi]
df_counts_mo[~df_counts_mo] = df_rand_mo[~df_counts_mo]
df_counts_vi[df_counts_vi == 1] = 0
df_counts_mo[df_counts_mo == 1] = 0
...
#     #remove values below threshold x
#     df_train_vivant[df_train_vivant < x] = 0
#     df_train_mort[df_train_mort < x] = 0
    
#     #most zeros in row without taking sliding window
#     max_zero_vi = max_zero(df_train_vivant)
#     max_zero_mo = max_zero(df_train_mort)
    
#     #use window (1) - ignore one value spikes (peaks)
#     slide_max_zero_vi = slide_max_zero(df_train_vivant)
...
# #subtraction of blue red curves
# plt.figure()
# plt.plot(xrange,(df_threshold['viv percent'] - df_threshold['mort percent']))
# plt.title('%Mort - %Vivant')

# # plot least zeros in dead values - max in living (negative means dead moves more than living...)
# plt.figure()
# plt.plot(xrange,df_threshold['Min mort'] - df_threshold['Max vivant'])
# plt.title('Min(mort) - Min(vivant)')

# # plot mean morts vs vivants for each threshold value
# plt.figure()
# plt.plot(xrange,df_threshold['mean mort'] - df_threshold['mean vivant'])
# plt.title('E(mort) - E(vivant)')

# #lower quantile of dead - top quantile in living
# plt.figure()
# plt.plot(xrange,df_threshold['q = 0.1 mort'] - df_threshold['q = 0.9 vivant'])
# plt.title('Q(0.1)mort - Q(0.9)vivant')
...
# df_dist = df_dist.drop('timestep',axis = 1)
